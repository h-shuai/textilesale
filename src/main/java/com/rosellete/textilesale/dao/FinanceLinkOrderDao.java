...
public interface FinanceLinkOrderDao extends BaseRepository<FinanceLinkOrder,String> {
    @Query(value = "select batch_no batchNo,sum(order_amount) totalAmount,create_date createDate from t_finance_link_order group by batch_no,create_date order by create_date desc",nativeQuery = true)
