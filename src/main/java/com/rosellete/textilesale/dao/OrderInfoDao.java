...
@Repository
public interface OrderInfoDao extends BaseRepository<OrderInfo, Integer> {
    ...
    List<OrderInfo> findOrderList(@Param("orderNo") Integer orderNo,@Param("customerNo") Integer customerNo, @Param("customerName") String customerName,
    ...
    List<OrderInfo> findPagedOrderList(@Param("orderNo") Integer orderNo,@Param("customerNo") Integer customerNo, @Param("customerName") String customerName,
                                       @Param("customerPhoneNo") String customerPhoneNo,
                                       @Param("orderStatus") String orderStatus,
                                       @Temporal(TemporalType.TIMESTAMP) Date startDate,
                                       @Temporal(TemporalType.TIMESTAMP) Date endDate,
    ...
    int updateOrderStatus(@Param("orderNo") Integer orderNo, @Param("orderStatus") String orderStatus,
    ...
    @Query(value = "update t_order_info t set t.order_status =?2, t.order_amount = ?3, t.update_user = ?4, t.update_date = NOW()" +
            " where t.order_no = ?1", nativeQuery = true)
    int updateStatusAndAmount(@Param("orderNo") Integer orderNo, @Param("orderStatus") String orderStatus, @Param("amount") Double amount, String updater);
    ...
    @Query(value = "select a.order_no orderNo,a.customer_no customerNo,c.name customerName,sum(b.product_length) productLength " +
            ...
            "and IF(?3 is not null, a.delivery_address = ?3, 1=1) " +
            "group by a.order_no,a.customer_no order by a.order_date desc", nativeQuery = true)
    List<Map<String, Object>> getWaitPackOrderList(@Param("orderNo") Integer OrderNo, @Param("customerNo") Integer customerNo, @Param("address") String address);
    ...
    List<String> getTotalCount(@Param("orderNo") List<Integer> orderNo,@Param("customer") String customer);
    ...
    List<Map<String, String>> getWaitPieceList(@Param("orderNo") List<Integer> orderNo);

    @Query(value = "select distinct delivery_address from t_order_info where order_no in ?1", nativeQuery = true)
    List<String> getAddressByOrderNos(@Param("orderNo") List<Integer> orderNo);
    ...
    List<Map<String,Object>> getWaitSettleList(@Param("orderNo") Integer orderNo, @Param("customerName") String customerName);

    @Transactional
    @Modifying
    @Query(value = "update t_order_info t set t.settle_status = 1 " +
            " where t.order_no in ?1", nativeQuery = true)
    int updateSettleStatus(@Param("orderNo") List<Integer> orderNo);

    @Query(value = "select ifnull(max(a.order_no),100000) as orderNO from t_order_info a ", nativeQuery = true)
    int findMaxOrderNo();
}
